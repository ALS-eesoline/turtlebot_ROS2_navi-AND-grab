...
class ArmController(Node):
    ...
    def release_b(self):
        release_pos = [np.deg2rad(0.0),np.deg2rad(18.),np.deg2rad(45.0),np.deg2rad(-20.0)]
        check_pos = copy.deepcopy(self.joint_pos)
        
        if self.B_POINT_release:
            self.set_group_pos(release_pos)
            if np.abs(check_pos[0]-release_pos[0])<0.02 and np.abs(check_pos[1]-release_pos[1])<0.02 and np.abs(check_pos[2]-release_pos[2])<0.02 and np.abs(check_pos[3]-release_pos[3])<0.02:
        ...
        if self.cnt%4 == 0:
            ...
            if np.abs(check_pos[0]-state[0]) < 0.02:
                if m_x > 0.25:
                    ...
                    if np.abs(check_pos[1]-state[1]) < 0.02:
                        self.set_single_pos("wrist_angle",state[3])
                        if np.abs(check_pos[3]-state[3]) < 0.1:
            ...
            if np.abs(state[0] - check_pos[0]) < thred and np.abs(state[1] - check_pos[1]) < thred and np.abs(state[2] - check_pos[2]) < thred and np.abs(state[3] - check_pos[3]) < thred:
                self.set_single_pos('gripper',0.7)
                self.cnt+=1
...
def main():
    ...
    # go to near S position
    # go_dest_by_navigation(0.27,-0.38,-face_y,face_y,navigator)
    home_pose_arm = [np.deg2rad(-90.0),np.deg2rad(-35.0),np.deg2rad(60.0),np.deg2rad(50.0)]
    contoller.set_group_pos(home_pose_arm)
    

    # go to near A position
    a_result = go_dest_by_navigation(0.20, -3.3, -face_y, face_y,navigator)
    if a_result == TaskResult.SUCCEEDED:
        ...
        while True:
            if not contoller.target_T_sd[9] == 0.0:
                # contoller.start_grabbing = True
                # 识别到了，开始矫正位置，准备抓取
                if contoller.finish_grabing:
                    contoller.start_grabbing = False
                    break
    ...
    go_dest_by_navigation(2.7,-3.2,0.0,1.0,navigator)          
