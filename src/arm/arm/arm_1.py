...
class ArmController(Node):
    def __init__(self):
        ...
        self.pub_timer = self.create_timer(10, self.timers_cb)
        ...
        self.initial_guesses[2][0] = np.deg2rad(0)
        # self.initial_guesses[3][0] = np.deg2rad(-10)
    ...
    def cam_pose(self,msg):
        ...
        if self.marker2arm() is not None:
            ...
            length = 10  # Adjust this to change the length
            num1 = -T_sd[0][2]
            num2 = -T_sd[1][2]
            num3 = -T_sd[2][2]
            # Printing the numbers with fixed length
            formatted_num1 = f"{num1:{length}.2f}"
            formatted_num2 = f"{num2:{length}.2f}"
            formatted_num3 = f"{num3:{length}.2f}"

            # print(f"Number 1: {formatted_num1}, Number 2: {formatted_num2}, Number 3: {formatted_num3}")
            print(np.arctan2(num2,num1))

    
            
            # mlist, mflag = self.matrix_control(T_sd)
            # mlist = [np.deg2rad(0),np.deg2rad(0),np.deg2rad(-10),np.deg2rad(-20)]
            # print(mlist)
            # self.arm_group_command.name = "arm"
            # self.arm_group_command.cmd = mlist
            # self.group_pub.publish(self.arm_group_command)
            
            # self.arm_command.name = "gripper"
            # self.arm_command.cmd = 1.0
    ...
    # demo
    def timers_cb(self):
        # kk = [np.deg2rad(90.0),np.deg2rad(-5.0),np.deg2rad(80.0),np.deg2rad(-80.0)]
        # kk = [0.0,np.deg2rad(15.0),np.deg2rad(50.0),np.deg2rad(-80.0)]
        # kk = [0.0,np.deg2rad(-10.0),np.deg2rad(55.0),np.deg2rad(40.0)]
        print("1")
        sweep_flag_init = -np.sign(self.joint_pos[0])
        # sweep_ready = [sweep_flag_init*1.5,np.deg2rad(-15.0),np.deg2rad(90.0),np.deg2rad(-50.0)]
        # self.set_group_pos([sweep_ready[0],sweep_ready[1],sweep_ready[2],sweep_ready[3]])
        # time.sleep(2.5)
        
        sweep_ready = [self.joint_pos[0],np.deg2rad(5.0),np.deg2rad(-90.0),np.deg2rad(0.0)]
        self.set_group_pos([sweep_ready[0],sweep_ready[1],sweep_ready[2],sweep_ready[3]])
        time.sleep(2.5)
        self.set_single_pos("waist",sweep_flag_init*1.5)
        time.sleep(2)
        sweep_ready = [sweep_flag_init*1.5,0.0,np.deg2rad(50.0),np.deg2rad(-20.0)]
        self.set_group_pos([sweep_ready[0],sweep_ready[1],sweep_ready[2],sweep_ready[3]])
        time.sleep(1)
        # sweep = [kk[0],kk[1],kk[2],kk[3]]
        # self.set_group_pos(sweep_ready)
