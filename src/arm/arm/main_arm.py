...
class ArmController(Node):
    def __init__(self):
        ...
        # self.cam_view = self.create_subscription(JointState,"/joint_states",self.press_callback,10)
    ...
    def timer_cb(self):
        ...
        if self.cnt%4 == 0:
            ...
            try:
                ...
                self.catch_xyz[0] = ave_x
                self.catch_xyz[1] = ave_y
                self.catch_xyz[2] = ave_z
            ...
            if np.abs(check_pos[0]-state[0]) < 0.02 or self.ifShake:
                self.ifShake = True
                if self.skakeCounter%2 == 0:
                    self.set_single_pos("waist",state[0]+0.05)
                else:
                    self.set_single_pos("waist",state[0]-0.05)
                self.skakeCounter += 1
                if m_x > 0.25:
                    ...
                    if np.abs(check_pos[3]-state[3]) < 0.02:
                        print(f"////////////////{check_pos[1]}///////{state[1]}")
                        self.set_single_pos("shoulder",state[1])
                        if np.abs(check_pos[1]-state[1]) < 0.40:
                            print("wrist moving")
                            # self.set_single_pos("elbow",state[2])
                            joint_list = [state[0],state[1],state[2], state[3]]  
                            self.set_group_pos(joint_list)
                    # joint_list = [state[0],state[1],state[2], state[3]]
                    # self.set_group_pos(joint_list)
            # if state[1] > 0.9 or state[1] < -2.0:
            #     print(f":::::::{state}::::::::")
                
            #     self.target_T_sd = [0.0]*10
            #     self.cnt = 0
            #     # self.initial_guesses[1][0] = np.deg2rad(0)
            #     self.initial_guesses[1][0] = random.uniform(-0.3, 0.8)
            #     home_pose_arm = [np.deg2rad(-90.0),np.deg2rad(-35.0),np.deg2rad(60.0),np.deg2rad(50.0)]
            #     self.set_group_pos(home_pose_arm)
            # if np.abs(state[0] - check_pos[0]) < thred and np.abs(state[1] - check_pos[1]) < thred and np.abs(state[2] - check_pos[2]) < thred and np.abs(state[3] - check_pos[3]) < thred:
            if np.abs(state[1] - check_pos[1]) < thred and np.abs(state[2] - check_pos[2]) < thred and np.abs(state[3] - check_pos[3]) < thred:
...
def move2Face(x, y, controller,initial_theta,initial_x,initial_y):
    msg = Twist()
    print(f"===={x}===={y}======")
    if controller.state_flag == 0:
        msg.angular.z = 0.15*(initial_theta - np.sign(y)*np.deg2rad(90)-controller.current_theta)
        # msg.angular.z = 0.2*(initial_theta -controller.current_theta)
        print(f"////{initial_theta}// {controller.state_flag}///{controller.current_theta}///")
        
        if np.abs(msg.angular.z) < 0.01:
            controller.state_flag += 1
    elif controller.state_flag == 1:
        msg.linear.x = -0.1*(np.sqrt((controller.current_x - initial_x)**2 + (controller.current_y - initial_y)**2) - y)
        print(f"////{msg.linear.x}//{controller.state_flag}//")
        
        if np.abs(msg.linear.x) < 0.001:
            controller.state_flag += 1
    elif controller.state_flag == 2:
        msg.angular.z = 0.2*(initial_theta - controller.current_theta)
        print(f"////{msg.angular.z}//{controller.state_flag}//")
        if np.abs(msg.angular.z) < 0.02:
            controller.state_flag += 1
            controller.initial_x_2 = controller.current_x
            controller.initial_y_2 = controller.current_y
    elif controller.state_flag == 3:
        msg.linear.x = -0.2*np.sqrt((controller.current_x - controller.initial_x_2)**2 + (controller.current_y - controller.initial_y_2)**2) - (x - 0.22)
        print(f"////{msg.linear.x}//{controller.state_flag}//")
        
        if np.abs(msg.linear.x) < 0.02:
            controller.state_flag += 1
    else: 
        pass
...
def main():
    ...
    if a_result == TaskResult.SUCCEEDED:
        ...
        while True:
            ...
            if not contoller.target_T_sd[24] == 0.0:
                # 识别到了，开始矫正位置，准备抓取
                print(2222222222222222222222222222222222)
                if init_flag:
                    goal_matrix = copy.deepcopy(contoller.target_T_sd)
                    for i in range(25):
                        tar_x += goal_matrix[i][0][3]
                        tar_y += goal_matrix[i][1][3]
                    tar_x = tar_x/25      
                    tar_y = tar_y/25        
                    initial_x = contoller.current_x
                    initial_y = contoller.current_y
                    initial_theta = contoller.current_theta
                    print(contoller.current_theta)
                    time.sleep(0.5)
                    init_flag = False
                ...
                if not go_face_falg:
                    ...
                    move2Face(tar_x,tar_y,controller=contoller,initial_theta=initial_theta,
                              initial_x=initial_x,initial_y = initial_y)
                    if contoller.state_flag== 4:
                        go_face_falg = True 
                        
                    
                    
                    # msg.angular.z = 1.2*(tar_y)
                    # msg.linear.x = 0.5*(tar_x - 0.20)
                    # print(f"v of x is {msg.linear.x},v of y is {msg.angular.z},")
                    # print(f"tar of x is {tar_x},tar of y is {tar_y},")
                    # contoller.twi_pub.publish(msg)
                    # if np.abs(tar_x) < 0.20 and np.abs(tar_y) < 0.05:
                else:
                ...
                pass
            #     modify_y -= 0.05
            #     go_dest_by_navigation(0.27,modify_y,face_y,face_y,navi=navigator)
            #     if fail_counter == 2:
            #         go_dest_by_navigation(0.27,-0.38,face_y,face_y,navi=navigator)
            #         break
            #     print("run in to the forward 5 cm")
            #     fail_counter += 1
            if contoller.finish_grabing:
                break

    go_dest_by_navigation(navigator.feedback.current_pose.pose.position.x,navigator.feedback.current_pose.pose.position.y,0.0,1.0,navi=navigator)
