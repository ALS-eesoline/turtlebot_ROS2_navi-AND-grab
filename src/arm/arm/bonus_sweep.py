...
def main():
    ...
    if a_result == TaskResult.SUCCEEDED:
        ...
        #             continue
        #         tar_pos_list = [waist_pose,np.deg2rad(-5.0),np.deg2rad(80.0),np.deg2rad(-80.0)]
        #         kk = [tar_pos_list[0],tar_pos_list[1],tar_pos_list[2],tar_pos_list[3]]
     ...
     # + 90 ~ -90; - -90 ~ 90
        ...
        time.sleep(0.5)
        sweep_flag_init = np.sign(contoller.target_T_sd[0][1][2])
        if_sweep = contoller.target_T_sd[0][1][2]
        ...
        home_pose_arm_kk = [-1.5,np.deg2rad(-10.0),np.deg2rad(55.0),np.deg2rad(0.0)]
