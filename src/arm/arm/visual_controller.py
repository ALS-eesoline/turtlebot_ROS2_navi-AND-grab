...
def main():
    ...
    if a_result == TaskResult.SUCCEEDED:
     ...
     # + 90 ~ -90; - -90 ~ 90
        ...
        M = copy.deepcopy(contoller.target_T_sd[0])
        ...
        while True:
            ...
            msgg.angular.z = (1) / (1+(delta_y/delta_x)**2)*(d_y/delta_x + delta_y*d_x/(delta_x**2))
            theta = np.arctan2(delta_y,delta_x)
            msgg.linear.x = (delta_x/np.cos(theta))*0.5
            
            now_x = x
            now_y = y
            contoller.twi_pub.publish(msgg)
            time.sleep(period)             
